...
\includegraphics[width=5.5in,height=3.5in]{report_files/figure-latex/dot-figure-1.png}

\subsubsection{Activity Diagram}\label{activity-diagram}

\includegraphics[width=5.5in,height=3.5in]{report_files/figure-latex/dot-figure-2.png}
...
characters result in a \texttt{SyntaxError}.
...
The \texttt{visitReturnStmt()} method throws a special kind of error
that remembers the value object that is being returned. That
\texttt{Returnable} error is then caught by the mentioned above
\texttt{Function} object during interpretation. The returned value is
extracted and returned back to the initial \texttt{callExpr}.
...
could be helpfull if we ever decided to expand the VVPL. /newline
...
we wouldn't have to worry about. Personally I think that using Try Catch
methods as a way of returing functions provides too big of an overhead.
/newline

Overally changing the general form of the interpreter and automation of
Scanners and Parsers could allow us to create a very verbose and
functional language, that is fast (compiled into LLVM, then either
interpreted or compiled to Machine Code using \textbf{gcc}) and easy to
modify to your needs. Possible new features include allowing user inputs
or explicit memory allocation.
